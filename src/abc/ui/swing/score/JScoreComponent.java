...
/**
 * This JComponent enables the display of a tune (its score). 
 */
public class JScoreComponent extends JComponent {
	...
	protected BufferedImage m_bufferedImage = new BufferedImage(3000, 3000, BufferedImage.TYPE_INT_RGB);;
	...
	/** Writes the tune score to a jpg file.
	...
	public void writeScoreTo(Tune tune, File file) throws IOException {
		...
		Dimension dim = drawIn(tune, (Graphics2D) score.getGraphics());
	...
	protected Dimension drawIn(Tune tune, Graphics2D g){
		if (tune!=null) {
			if (m_metrics==null)
				m_metrics = new ScoreMetrics(g);
	...
	private void initNewStaffLine(Graphics2D context, KeySignature key, TimeSignature ts, Point2D cursor, ScoreMetrics metrix){
		cursor.setLocation(0, cursor.getY()+staffLinesOffset);
		SClef clef = new SClef(cursor, metrix);
		double width = clef.render(context);
