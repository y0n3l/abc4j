...
class JGroupOfNotes extends JScoreElementAbstract {
	...
	public void setStemUp(boolean isUp) {
		for (int i=0; i<m_jNotes.length; i++) {
			((JNote)m_jNotes[i]).setStemUp(isUp);
		}
	}
...
/* TJM */
		JGroupableNote lowestNote = m_jNotes[Note.getLowestNoteIndex(m_notes)];
		// assume every note in group has same auto stemming policy
// can be indivudual beamed note or chord multinote

		boolean autoStemming = true;
		((JNote)highestNote).getAutoStem();

System.out.println("line 118:"+highestNote.getClass().getName());


		// corrected stem directions
		if ( ((Note)highestNote.getMusicElement()).getHeight()<Note.B) {
			if (autoStemming) setStemUp(true);
		} else if ( ((Note)lowestNote.getMusicElement()).getHeight()>=Note.B) {
			if (autoStemming) setStemUp(false);
		} else {
			byte h = ((Note)highestNote.getMusicElement()).getHeight();
			byte l = ((Note)lowestNote.getMusicElement()).getHeight();
		...
		boolean isUp = ((JNote)m_jNotes[0]).hasStemUp();
		...
		if (isUp) {
			...
			m_stemYend = (int)highestNote.getStemBegin().getY()-m_metrics.getStemLength(NOTATION_CONTEXT);
			...
			//based on this, calculate the new stem Y end.
			m_stemYend = (int)lowestNote.getStemBegin().getY()+m_metrics.getStemLength(NOTATION_CONTEXT);
		}
