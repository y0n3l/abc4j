...
class JNote extends JNoteElementAbstract {
	...
	/** Sets the Unicode value of the given note as a char array.
	 * @param the note instance
	 * @see #noteChars
 	 */
	protected char[] valuateNoteChars(Note noteToValuate) {
		...
		if (note.isRest()){
			valuatedNoteChars = m_metrics.getRestChar(noteDuration);
			//System.out.println("duration of the rest is " + noteDuration);
		}
		else {
			if (isStemUp())
				valuatedNoteChars = m_metrics.getNoteStemUpChar(noteDuration);
			else
				valuatedNoteChars = m_metrics.getNoteStemDownChar(noteDuration);
		}
