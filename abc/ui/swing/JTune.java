...
/* TJM */
	...
	//temporary variables used only to compute the score when tune is set.
...
/* TJM : start */		NoteAbstract note = (NoteAbstract) s;
						if (note.hasGracingNotes()) {
							Note [] graceNotes = note.getGracingNotes();
							if (graceNotes.length > 0 ) {
								// change gracenote duration, appearance, etc.
								// do this at all? here? in JGroupOfGracings ?
								for (int j=0; j<graceNotes.length; j++) {
//									graceNotes[j].setStrictDuration(Note.THIRTY_SECOND);
//									graceNotes[j].setDuration(Note.THIRTY_SECOND);
								}
								appendGracingsToScore(graceNotes);
							}
						}
/* TJM : end */
...
/* TJM */
	...
	protected void appendToScore(JScoreElementAbstract element) {
		...
		/* TJM */
		// increment width by a fixed space
		// NOTE: this should be controlled by a UI variable - either fixed spacing
		//       or variable spacing, based on note duration.
		if (element instanceof JGraceNote) {
		  cursorNewLocationX += (int)m_metrics.getGraceNotesSpacing();
		} else {
		  cursorNewLocationX += (int)m_metrics.getNotesSpacing();
		}
...
/* TJM */
	private void appendGracingsToScore(Note[] notes) {
		if (notes.length==1) {
			//JScoreElementAbstract renditionResult = null;
			JGraceNote renditionResult = new JGraceNote(notes[0], cursor, m_metrics);
			appendToScore(renditionResult);
		} else if (notes.length>1) {
			JScoreElementAbstract renditionResult = null;
			renditionResult = new JGroupOfGracings(m_metrics, cursor, notes);
			appendToScore(renditionResult);
		}
	}
...
/* TJM */
/* Issue 14 bug fixes */
				// is it over or under notes?
				// have to use JNote element to determine which!
				// JNote cast should be safe ......
				boolean isOver = true;
				JNote jn = (JNote)m_scoreElements.get(n);
				if (jn.hasStemUp()) {
					isOver = false;
				}
				if (isOver) {
			...
			if (elmtStart.getStaffLine().equals(elmtEnd.getStaffLine())) {
				...
				if (lowestNote.equals(slurDef.getStart())) {
					controlPoint = new Point2D.Double(
							elmtStart.getSlurUpAnchor().getX()+ (elmtEnd.getSlurUpAnchor().getX()-elmtStart.getSlurUpAnchor().getX())/2,
							elmtStart.getSlurUpAnchor().getY()+ m_metrics.getNoteWidth()/4);
				} else
				if (lowestNote.equals(slurDef.getEnd())) {
					controlPoint = new Point2D.Double(
							elmtStart.getSlurUpAnchor().getX()+ (elmtEnd.getSlurUpAnchor().getX()-elmtStart.getSlurUpAnchor().getX())/2,
							elmtEnd.getSlurUpAnchor().getY()+ m_metrics.getNoteWidth()/4);

				} else {
					JNote lowestNoteGlyph = (JNote)m_scoreElements.get(lowestNote);
					controlPoint = new Point2D.Double(lowestNoteGlyph.getSlurUpAnchor().getX(),
							lowestNoteGlyph.getSlurUpAnchor().getY() + m_metrics.getNoteWidth()/2);
				}
				GeneralPath path = new GeneralPath();
				path.moveTo((int)elmtStart.getSlurUpAnchor().getX(), (int)elmtStart.getSlurUpAnchor().getY());
				QuadCurve2D q = new QuadCurve2D.Float();
				q.setCurve(
						elmtStart.getSlurUpAnchor(),
						newControlUp(elmtStart.getSlurUpAnchor(), controlPoint, elmtEnd.getSlurUpAnchor()),
						elmtEnd.getSlurUpAnchor());
				path.append(q, true);
	...
	private JStaffLine initNewStaffLine() {
		...
		cursor.setLocation(MARGIN_LEFT, cursor.getY() + m_staffLinesSpacing);
	...
	private Point2D newControlUp (Point2D start, Point2D ctrl, Point2D end) {
	        Point2D.Double newCtrl = new Point2D.Double();
	        newCtrl.x = 2 * ctrl.getX() - (start.getX() + end.getX()) / 2;
	        newCtrl.y = /*2 * */ctrl.getY() - (start.getY() + end.getY()) / 12;
	        return newCtrl;
	    }
